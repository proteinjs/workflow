...
export const WorkflowExecutionTrackerBreadcrumbs = ({
  ...
  function stepsToDisplay() {
    ...
    const displaySteps = [];
    const startIndex =
      currentStepIndex == 0 ? 0 : currentStepIndex == steps.length - 1 ? currentStepIndex - 2 : currentStepIndex - 1;
    const endIndex =
      startIndex + (maxStepsToDisplay - 1) < steps.length ? startIndex + (maxStepsToDisplay - 1) : steps.length - 1;
