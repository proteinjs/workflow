...
export class KnexSchemaMetadata implements SchemaMetadata {
  ...
  async tableExists(table: Table<any>): Promise<boolean> {
		return await this.knexDriver.getKnex().schema.withSchema(this.knexDriver.getDbName()).hasTable(table.name);
	}

  async columnExists(columnName: string, table: Table<any>): Promise<boolean> {
    const result = await this.knexDriver.getKnex().withSchema('INFORMATION_SCHEMA').select().from('COLUMNS').where({
  ...
  async getPrimaryKey(table: Table<any>) {
    const result = await this.knexDriver.getKnex().withSchema('INFORMATION_SCHEMA').select().from('KEY_COLUMN_USAGE').where({
      'TABLE_SCHEMA': this.knexDriver.getDbName(),
      'TABLE_NAME': table.name,
  ...
  async getIndexes(table: Table<any>) {
    const result = await this.knexDriver.getKnex().raw(`SHOW INDEX FROM ${this.knexDriver.getDbName()}.${table.name}`);
