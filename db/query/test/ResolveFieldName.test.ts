...
describe('QueryBuilder - Resolve Field Name Support', () => {
  ...
  test('convert camelCase field names to snake_case', () => {
    ...
    const resolveFieldName = (propertyName: string): string => {
      return propertyName.replace(/\.?([A-Z]+)/g, (x, y) => `_${y.toLowerCase()}`).replace(/^_/, "");
    };
    const qb = new QueryBuilder<Employee>(tableName, resolveFieldName)
    ...
    // Standard SQL output
    expect(qb.toSql().sql).toBe("SELECT MAX(employee_age) FROM Employee WHERE (((department = 'Engineering' AND employee_name LIKE '%John%') OR department = 'Product') AND (employee_age > 30 AND employee_age < 50) AND (employee_salary > 50000 OR employee_salary <= 100000)) GROUP BY employee_age;");
  
    // SQL output with positional parameters
    let result = qb.toSql({ useParams: true });
    expect(result.sql).toBe("SELECT MAX(employee_age) FROM Employee WHERE (((department = ? AND employee_name LIKE ?) OR department = ?) AND (employee_age > ? AND employee_age < ?) AND (employee_salary > ? OR employee_salary <= ?)) GROUP BY employee_age;");
    ...
    expect(result.sql).toBe("SELECT MAX(employee_age) FROM Employee WHERE (((department = @param0 AND employee_name LIKE @param1) OR department = @param2) AND (employee_age > @param3 AND employee_age < @param4) AND (employee_salary > @param5 OR employee_salary <= @param6)) GROUP BY employee_age;");
